// ============================================================
// 左侧文件浏览器 (fexp) — v1.6.1 (DSH 静态 bundle 插件 · Client 半区)
...
//   - host.call(method, args) → fetch 同源路由 /fexp/<method>(Host 半区
//     在 lib/index.js 通过 webServer 挂载, 行为与动态 RPC 一致)
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function insertCss(css) {
      ...
      el.setAttribute('data-plugin', 'fexp-file-explorer')
      el.textContent = css
    ...
    async function hostCall(method, args) {
      const response = await fetch('/fexp/' + method, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify(args || {}),
      })
      if (!response.ok) throw new Error('fexp: HTTP ' + response.status)
    ...
    `)
      ...
      async function safeCall(method, args) {
        try {
          return await hostCall(method, args)
        } catch (err) {
